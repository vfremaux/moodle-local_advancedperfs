...
class core_component {
    ...
    protected static function init() {
        ...
        if (!empty($CFG->alternative_component_cache)) {
            ...
            // PATCH+. : let pass and just destroy physical cache.
...
';
    ...
    public static function get_plugin_list_with_file($plugintype, $file, $include = false) {
		...
		// PATCH+ : Enhance deep faults debugging.
		// @see : local_advancedperfs
        if (!empty($CFG->harddebug)) {
            echo "Scanning for $plugintype<br/>";
        }
		// PATCH-.
        $pluginfiles = array();
        ...
        if ($include) {
            foreach ($pluginfiles as $path) {
            	// PATCH+ : Allows hardebugging of early errors at setup time.
            	// @see local_advancedperfs
                if (!empty($CFG->harddebug)) {
...
// @see local_advancedperfs
    public static function get_component_classes_in_namespace($component, $namespace = '') {
        ...
        if (!isset($sortedclassmap)) {
            foreach (self::$classmap as $key => $value) {
                $parts = explode("\\", $key);
                $componentpart = array_shift($parts);
                $componentpartkey = implode("\\", $parts);
                $sortedclassmap[$componentpart][$componentpartkey] = $value;
            }
        }

        if (empty($sortedclassmap[$component])) {
            return array();
        }
        // $it = new RegexIterator(new ArrayIterator(self::$classmap), $regex, RegexIterator::GET_MATCH, RegexIterator::USE_KEY);
        // $it = new RegexIterator(new ArrayIterator($sortedclassmap[$component]), $regex, RegexIterator::GET_MATCH, RegexIterator::USE_KEY);
        $it = array_intersect_key($sortedclassmap[$component], array_flip(preg_grep($regex, array_keys($sortedclassmap[$component]), false)));
